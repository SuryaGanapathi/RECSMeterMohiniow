...
import com.recsmeterreading.activities.PendingBillActivity;
import com.recsmeterreading.connections.Constants;
...
public class Utils {
    ...
    public static GetBillDetailsModel generateBillModel = new GetBillDetailsModel();
    // storage path to create db
    public ArrayList<GetBillDetailsModel> getBillDetailsModel = new ArrayList<>();

    public String databasePath = Environment.getExternalStorageDirectory().getAbsolutePath() + File.separator + "com.recs/Database_"+ Constants.VERSION_ID+"/";
    public String databaseName = "RecsBillingData";


    public static Utils getSharedInstance() {
        if (sharedInstance == null) {
            sharedInstance = new Utils();
        }
        return sharedInstance;
    }

    /*creating db folders in storage and creating db.*/
    public void createFoldersAndDb() {
        File databasePath = new File(Utils.getSharedInstance().databasePath);
    ...
    public void whenInternetisEnabled(final Context context, boolean isConnected) {

        if (isConnected) {
            for (int i = 0;i<Utils.getSharedInstance().getBillDetailsModel.size();i++) {
                Utils.getSharedInstance().getBillDetailsModel.remove(i);
                PendingBillActivity.pendingBillAdapter.notifyDataSetChanged();
            }
           // Toast.makeText(context, "Pending sync api call started", Toast.LENGTH_SHORT).show();
        }
    }
